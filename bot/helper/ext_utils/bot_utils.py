        ...
        for download in list(download_dict.values()):
            if download.status() == MirrorStatus.STATUS_DOWNLOADING:
                ...
                if 'K' in spd:
                    ...
                    up_speed += float(spd.split('M')[0]) * 1048576
        bmsg = f"<b>🖥پردازنده:</b> {cpu_percent()}% | <b>💿فضای آزاد:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
        bmsg += f"\n<b>⚙️رم:</b> {virtual_memory().percent}% | <b>〽️آپتایم:</b> {get_readable_time(time() - botStartTime)}"
        bmsg += f"\n<b>🔻دانلود:</b> {get_readable_file_size(dl_speed)}/s | <b>🔺آپلود:</b> {get_readable_file_size(up_speed)}/s"
